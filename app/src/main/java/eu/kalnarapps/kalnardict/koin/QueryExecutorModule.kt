...
val queryExecutorModule = module {
    ...
    single<QueryExecutor>(named(QueryMode.MATCH_ANYWHERE.name)) {
        RoomQueryExecutor(
            wordDao = get(),
    ...
    single<QueryExecutor>(named(QueryMode.MATCH_BEGINNING.name)) {
        RoomQueryExecutor(
            wordDao = get(),
    ...
    single<QueryExecutor>(named(QueryMode.MATCH_END.name)) {
        RoomQueryExecutor(
            wordDao = get(),
    ...
    single<QueryExecutor>(named(QueryMode.MATCH_FUZZY.name)) {
        RoomQueryExecutor(
            wordDao = get(),
    ...
    single<QueryExecutor>(named(QueryMode.MATCH_EXACT.name)) {
        RoomQueryExecutor(
            wordDao = get(),
